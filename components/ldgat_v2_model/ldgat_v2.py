"""This module contains a class with an implementation of the LDGATv2 model."""
...
class LDGATv2(nn.Module):
    """Class with an implementation of a model based on the LDGCNN architecture with a graph attention mechanism.

    This model uses a second version of the graph attention mechanism (check https://arxiv.org/abs/2105.14491
    and https://pytorch-geometric.readthedocs.io/en/latest/generated/torch_geometric.nn.conv.GATv2Conv.html
    for details).
    ...
    def calculate_features(self, data) -> Tuple[torch.Tensor, torch.Tensor]:
        """Performs a forward pass of the model and returns the high-level features.

        Args:
            data: DataBatch.

        Returns:
            Two torch.Tensors - calculated local features before concatenation with the size of 128,
             and global features with the size of 1024.
        """
        if data.pos is not None and data.x is not None:
            x, pos, batch = data.x, data.pos, data.batch
            # x0 is (num_points, in_channels)
        ...
        # -> (num_points, in_channels + 64 + 64 + 64 + 128 + 1024)
        local_global_features = torch.cat([link_4, global_features_repeated], axis=1)
        return link_4, local_global_features
